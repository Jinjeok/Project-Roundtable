function recentContext(messages, count) {
  return (messages || [])
...
export class PromptBuilder {
  build(sessionState, playerInput) {
    const summaryBlock = sessionState.summary
...
${summaryBlock}최근 대화:
${recentContext(sessionState.messages, 5)}
...
Warden으로서 응답하세요. 판정이나 굴림이 필요하면 적절한 함수를 호출하세요.`;
  ...
  buildFollowUp(sessionState, functionName, result) {
    ...
    const followUpPrompt = `당신은 Cairn RPG의 Warden입니다.
...
위 대화 맥락을 유지하며 방금 주사위 결과가 실제 상황에 어떤 영향을 미쳤는지 묘사하세요. 생생하지만 간결하게. 필요하다면 현재 장면을 업데이트하세요.
**반드시 한국어로 응답하세요.**`;
